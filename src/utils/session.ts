...
export async function CreateSession(payload: Session2): Promise<void> {
...
export async function updateTokens({
  ...
  const { payload } = await jwtVerify<Session2>(cookie, encodedKey);

  if (!payload) throw new Error("Session not found");

  const newPayload: Session2 = {
    user: {
      ...payload.user,
    },
    accessToken,
